...
var styles = ['<%= lowerWidgetName %>.css'];
var scripts = ['<%= upperWidgetName %>.js'];
var templates = ['<%= lowerWidgetName %>.html'];

gulp.task('default', function() {
    var sourcesStream = gulp.src(scripts);

    var templatesStream = gulp.src(templates)
        .pipe(html2jsobject('nsGmx.Templates.<%= upperWidgetName %>'))
        .pipe(concat('templates.js'))
        .pipe(header('nsGmx.Templates.<%= upperWidgetName %> = {};\n'))
    ...
    var jsStream = streamqueue({
        ...
        .pipe(footer(';'))
        .pipe(concat('<%= lowerWidgetName %>.js'));
